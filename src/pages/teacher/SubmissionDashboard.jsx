...
const SubmissionDashboard = () => {
  ...
  const releaseScores = async (submission = null) => {
    ...
    try {
      ...
      showAlert(submission ? 'Score released.' : `${response.data.released_count} graded scores released.`, { variant: 'success' });
      setInsights(null);
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <div className="mb-5 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="grid gap-8 md:grid-cols-2 xl:grid-cols-4">
        <label className='text-sm' >From<input type="date" value={value('submitted_from')} onChange={(event) => updateParams({ submitted_from: event.target.value })} className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal" /></label>
        <label className='text-sm'>To<input type="date" value={value('submitted_to')} onChange={(event) => updateParams({ submitted_to: event.target.value })} className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal" /></label>
        <label className="relative block xl:col-span-2 mt-5"><span className="sr-only">Search submissions</span><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search by name, ID, or email" className="w-full rounded-lg border border-slate-300 py-2.5 pl-10 pr-3 text-sm outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100" /></label>
    ...
    <div className="mb-4 flex flex-wrap items-center gap-3">
      <span className="inline-flex" title={!value('quiz_id') ? 'Select a quiz to release all graded scores' : undefined}>
        <button disabled={!canReleaseQuiz || releasing || loading} onClick={() => releaseScores()} className="cursor-pointer rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white disabled:pointer-events-none disabled:opacity-40">{releasing ? 'Releasing…' : 'Release scores'}</button>
      </span>
    </div>
